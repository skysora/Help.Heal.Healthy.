...
//            }
    ...
    func backFromBackground() {
        if state == .scanning {
            let title = "警告：掃描可能已損壞"
            let message = "掃描被中斷了。 建議重新啟動掃描。"
            let buttonTitle = "重新掃描"
    ...
    @IBAction func addScanButtonTapped(_ sender: Any) {
        guard state == /*.testing*/.finish else { return }
    ...
    func showAlert(title: String, message: String, buttonTitle: String? = "OK", showCancel: Bool = false, buttonHandler: ((UIAlertAction) -> Void)? = nil) {
        ...
        if showCancel {
            actions.append(UIAlertAction(title: "關閉", style: .cancel))
        }
                ...
                if let scan = scan {
                    ...
                    case .defineBoundingBox, .scanning, .adjustingOrigin:
                        if reason == .relocalizing {
                            ...
                            print("警告：ARKit正在重新定位")
                            
                            let title = "警告：掃描可能已損壞"
                            let message = "跟踪發生了差距。 建議重新啟動掃描。"
                            let buttonTitle = "重啟掃描"
                            self.showAlert(title: title, message: message, buttonTitle: buttonTitle, showCancel: true) { _ in
                                self.state = .notReady
                            }
...
//    }
    ...
    func boundingBoxPositionOrExtentChanged(_ notification: Notification) {
        ...
        let xString = String(format: "寬: %.2f", box.extent.x)
        let yString = String(format: "高: %.2f", box.extent.y)
        let zString = String(format: "長: %.2f", box.extent.z)
        let distanceFromCamera = String(format: "%.2f m", distance(box.simdWorldPosition, cameraPos))
        displayMessage("距離: \(distanceFromCamera) \n\(xString) \(yString) \(zString)", expirationTime: 1.5)
        //"Current bounding box: \(distanceFromCamera) away\n\(xString) \(yString) \(zString)"
        print("邊框: \(distanceFromCamera) away\n\(xString) \(yString) \(zString)")
    }
    ...
    func displayWarningIfInLowPowerMode() {
        if ProcessInfo.processInfo.isLowPowerModeEnabled {
            let title = "低功耗模式已啟用"
            let message = "性能可能會受到影響。 要獲得最佳掃描結果，請在“設置”>“電池”中禁用低功耗模式，然後重新開始掃描"
            let buttonTitle = "OK"

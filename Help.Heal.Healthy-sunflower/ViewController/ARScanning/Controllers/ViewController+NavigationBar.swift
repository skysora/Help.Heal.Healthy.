...
extension ViewController {
    ...
    func setupNavigationBar() {
        backButton = UIBarButtonItem(title: "返回", style: .plain, target: self,
                                     action: #selector(previousButtonTapped(_:)))
        mergeScanButton = UIBarButtonItem(title: "合併掃描......", style: .plain, target: self,
                                          action: #selector(addScanButtonTapped(_:)))
        let startOverButton = UIBarButtonItem(title: "重新開始", style: .plain, target: self,
                                              action: #selector(restartButtonTapped(_:)))
        let navigationItem = UINavigationItem(title: "開始")

...
extension ViewController {
    ...
    func updateSessionInfoLabel(for trackingState: ARCamera.TrackingState) {
        ...
        var message: String = ""
        let stateString = state == /*.testing*/.finish ? "偵測中" : "掃描中"
        
        switch trackingState {
            
        case .notAvailable:
            message = "\(stateString) 不可能： \(trackingState.presentationString)"
            startTimeOfLastMessage = Date().timeIntervalSince1970
            expirationTimeOfLastMessage = 3.0
            
        case .limited:
            message = "\(stateString) 可能不起作用： \(trackingState.presentationString)"
            startTimeOfLastMessage = Date().timeIntervalSince1970
            expirationTimeOfLastMessage = 3.0
